<template>
  <loading-tap />
  <top-detail :operandata="banner" />
  <v-container class="mt-2">
    <breadcrumbs />
    <v-row>
      <v-col cols="12" md="6">
        <v-img
          src="https://cdn.aresa-digital.com/images/produk/heavy-duty/drive-in/drive-in-racking-TAP_3.webp"
          lazy-img="https://cdn.aresa-digital.com/images/produk/heavy-duty/drive-in/drive-in-racking-TAP_4.webp"
          alt="Rak Drive In"
          class="rounded-lg"
          cover
          width="100%"
          height="100%"
        />
      </v-col>

      <v-col cols="12" md="6">
        <h1 class="barlow">DRIVE IN RACKING</h1>
        <v-divider thickness="3" color="#FF3A3A" class="mb-3 opacity-100" />

        <p class="text-body-1 barlow2 text-justify">
          <span class="barlow">Drive-In Racking</span>
          adalah sistem penyimpanan palet yang dirancang untuk memaksimalkan
          kepadatan penyimpanan dengan menghilangkan lorong (aisle) antar rak.
          Sistem ini memungkinkan forklift masuk ke dalam rak untuk menyimpan
          atau mengambil palet, sehingga sangat cocok untuk penyimpanan stok
          massal dengan rotasi rendah.
        </p>
        <br />

        <h2 class="barlow2">Aplikasi di Industri:</h2>

        <p class="barlow2 mt-3 text-justify">
          <span class="barlow">Drive-In Racking</span>
          cocok untuk industri yang membutuhkan penyimpanan stok massal dengan
          rotasi rendah. Beberapa aplikasinya meliputi:
        </p>

        <ol class="ml-6 text-justify barlow2">
          <li class="my-3">
            <span class="barlow">Penyimpanan Bahan Baku: </span>
            Digunakan untuk menyimpan bahan baku dalam jumlah besar yang tidak
            perlu diakses secara terus-menerus, seperti di industri manufaktur.
          </li>

          <li>
            <span class="barlow">Gudang Dingin (Cold Storage): </span>
            Ideal untuk menyimpan produk beku atau makanan yang memerlukan
            penyimpanan jangka panjang.
          </li>

          <li class="my-3">
            <span class="barlow">Industri Minuman: </span>
            Cocok untuk menyimpan minuman dalam kemasan palet, seperti bir atau
            minuman ringan, yang disimpan dalam jumlah besar.
          </li>

          <li>
            <span class="barlow">Logistik dan Distribusi: </span>
            Digunakan untuk menyimpan produk dengan rotasi stok rendah, seperti
            barang musiman atau stok cadangan.
          </li>
        </ol>
      </v-col>
    </v-row>

    <v-row>
      <v-col cols="12" sm="6" class="px-4">
        <h2 class="barlow2">Keuntungan Drive-In Racking:</h2>

        <ol class="ml-6 text-justify barlow2">
          <li class="my-3">
            <span class="barlow">Kepadatan Penyimpanan Lebih Tinggi: </span>
            Menghilangkan lorong antar rak memungkinkan penyimpanan lebih banyak
            palet dalam area yang sama, sehingga ideal untuk gudang dengan ruang
            terbatas.
          </li>

          <li class="my-3">
            <span class="barlow">Efisiensi Ruang: </span>
            Maksimalisasi penggunaan ruang vertikal dan horizontal membuat
            sistem ini sangat efisien untuk penyimpanan stok massal.
          </li>

          <li>
            <span class="barlow">Biaya Ruang Lebih Rendah: </span>
            Mengurangi kebutuhan ruang lantai, yang dapat menurunkan biaya sewa
            atau konstruksi gudang.
          </li>

          <li class="mt-3">
            <span class="barlow">Cocok untuk Stok Massal: </span>
            Ideal untuk produk dengan rotasi stok rendah atau stok yang disimpan
            dalam jumlah besar untuk jangka waktu panjang.
          </li>
        </ol>
      </v-col>

      <v-col cols="12" sm="6" class="px-4">
        <h2 class="barlow2">Tantangan Drive-In Racking:</h2>

        <ol class="ml-6 text-justify barlow2">
          <li class="my-3">
            <span class="barlow">Akses Terbatas: </span>
            Hanya palet di bagian depan yang dapat diakses langsung, sementara
            palet di belakang hanya dapat diambil setelah palet depan
            dipindahkan. Ini membuat sistem ini kurang fleksibel untuk operasi
            dengan rotasi stok tinggi.
          </li>

          <li class="my-3">
            <span class="barlow">Risiko Kerusakan: </span>
            Forklift yang masuk ke dalam lorong rak berisiko menyebabkan
            kerusakan pada rak atau palet jika tidak dioperasikan dengan
            hati-hati.
          </li>

          <li>
            <span class="barlow">Kebutuhan Forklift Khusus: </span>
            Memerlukan forklift dengan kemampuan manuver yang baik dan operator
            yang terampil.
          </li>

          <li class="mt-3">
            <span class="barlow">Tidak Cocok untuk Semua Produk:</span>
            Kurang ideal untuk produk dengan tanggal kedaluwarsa atau yang
            memerlukan rotasi stok tinggi (FIFO).
          </li>
        </ol>
      </v-col>
    </v-row>

    <v-row>
      <v-col class="px-4">
        <v-divider thickness="3" color="#FF3A3A" class="mb-3 opacity-100" />
        <h1 class="barlow">Kesimpulan</h1>

        <p class="barlow2 text-h6 mt-3 text-justify">
          <span class="barlow">Drive-In Racking</span>
          adalah solusi penyimpanan yang sangat efisien untuk meningkatkan
          kepadatan penyimpanan dan mengoptimalkan penggunaan ruang gudang.
          Sistem ini cocok untuk produk dengan rotasi stok rendah dan gudang
          yang memprioritaskan kapasitas penyimpanan. Namun, kekurangan utamanya
          adalah akses terbatas ke palet di belakang, sehingga perlu
          dipertimbangkan sesuai dengan kebutuhan operasional gudang. Pemilihan
          sistem ini harus didasarkan pada jenis produk, frekuensi pengambilan,
          dan ketersediaan peralatan forklift yang sesuai.
        </p>
      </v-col>
    </v-row>
  </v-container>
  <chat-ai />
</template>

<script setup>
const banner = {
  judul: "DETAIL PRODUK",
};

useSeoMeta({
  title: "Rak Drive In",
  ogtitle: "Rak Drive In",
  description:
    "Rak Gudang Heavy Duty dengan sistem Drive In, ideal untuk penyimpanan barang dalam jumlah besar dan efisien di ruang terbatas.",
  ogDescription:
    "Rak Gudang Heavy Duty dengan sistem Drive In, ideal untuk penyimpanan barang dalam jumlah besar dan efisien di ruang terbatas.",
  keywords:
    "rak gudang, heavy duty rack, drive in rack, rak penyimpanan, rak industri, warehouse shelving, storage rack, pallet rack",
  ogImage:
    "https://cdn.aresa-digital.com/images/produk/heavy-duty/drive-in/drive-in-racking-TAP_og.webp",
  twitterCard: "summary_large_image",
  ogType: "product",
  ogUrl: "https://www.perkasaracking.co.id/produk/heavy-duty/drive-in",
  robots: "index, follow",
  canonical: "https://www.perkasaracking.co.id/produk/heavy-duty/drive-in",
});
</script>

<style scoped>
.barlow {
  font-family: "Barlow Semi Condensed", serif;
  font-weight: 700;
  font-style: normal;
}

.barlow2 {
  font-family: "Barlow Semi Condensed", serif;
  font-style: normal;
}
</style>
